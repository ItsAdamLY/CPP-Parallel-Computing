...
int main(int argc, char **argv) {
  ...
  if (rank == 0) 
  {
    ...
    MPI_Send(&other_value, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
    MPI_Recv(&local_value, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    std::cout << "Sum of 2 values: " << local_value+other_value  << std::endl;
  }
  else if (rank == 1)
  {
    ...
    other_value = atoi(argv[1]);
    MPI_Send(&other_value, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
    std::cout << "Product of 2 values: " << local_value*other_value << std::endl;
  }
